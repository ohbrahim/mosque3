    ...
    if (!$csrfValid) {
        ...
        if ($action === 'reply') {
            ...
            if ($db->update('messages', $data, 'id = ?', [$id])) {
                ...
                if ($messageData && function_exists('sendEmail')) {
...
if ($action === 'mark_read' && isset($_GET['id'])) {
    $id = (int)$_GET['id'];
    if ($db->update('messages', ['status' => 'read', 'updated_at' => date('Y-m-d H:i:s')], 'id = ?', [$id])) {
...
if (!function_exists('convertToArabicNumbers')) {
    function convertToArabicNumbers($number) {
        return $number; // يمكن تحسينها لاحقاً
    }
}
?>
<!DOCTYPE html>
                ...
                <div class="sidebar">
                    <div class="p-4">
                        ...
                        <nav class="nav flex-column">
                            ...
                            <a class="nav-link" href="../debug_contact_system.php" target="_blank">
            ...
            <div class="col-lg-10">
                <div class="main-content">
                    ...
                    <div class="debug-info">
                        ...
                        <p><strong>إجمالي الرسائل:</strong> <?php echo isset($stats) ? $stats['total'] : 'غير محدد'; ?></p>
                        <p><strong>آخر فحص:</strong> <?php echo date('Y-m-d H:i:s'); ?></p>
                        <p><strong>حالة قاعدة البيانات:</strong> 
                            <?php 
                            try {
                                $db->fetchOne("SELECT 1");
                                echo '<span class="text-success">متصلة</span>';
                            } catch (Exception $e) {
                                echo '<span class="text-danger">خطأ في الاتصال</span>';
                            }
                            ?>
                        </p>
                    </div>
                    ...
                    <div class="content-card">
                        <?php if ($action === 'list'): ?>
                            ...
                            <?php if (empty($messages)): ?>
                                <div class="alert alert-info text-center">
                                    ...
                                    <h4>لا توجد رسائل</h4>
                                    <p>لم يتم العثور على أي رسائل في قاعدة البيانات.</p>
                                    <a href="../contact_debug.php" class="btn btn-primary">اختبار صفحة التواصل</a>
                                </div>
                            <?php else: ?>
                                    ...
                                    <table class="table table-hover">
                                        ...
                                        <tbody>
                                            ...
                                            <tr class="message-row <?php echo $msg['status'] === 'unread' ? 'unread' : ''; ?>">
                                                ...
                                                <td>
                                                    <strong><?php echo htmlspecialchars($msg['sender_name'] ?? 'غير محدد'); ?></strong>
                                                    <br><small class="text-muted"><?php echo htmlspecialchars($msg['sender_email'] ?? 'غير محدد'); ?></small>
                                                    <?php if (!empty($msg['sender_phone'])): ?>
                                                        <br><small class="text-muted"><?php echo htmlspecialchars($msg['sender_phone']); ?></small>
                                                    <?php endif; ?>
                                                </td>
                                                <td>
                                                    <strong><?php echo htmlspecialchars($msg['subject'] ?? 'بدون موضوع'); ?></strong>
                        ...
                        <?php elseif ($action === 'view' && isset($messageData)): ?>
                            <!-- View Message -->
                            <div class="row">
                                <div class="col-md-8">
                                    <h4><?php echo htmlspecialchars($messageData['subject'] ?? 'بدون موضوع'); ?></h4>
                                    
                                    <div class="mb-3">
                                        <strong>من:</strong> <?php echo htmlspecialchars($messageData['sender_name'] ?? 'غير محدد'); ?>
                                        (<?php echo htmlspecialchars($messageData['sender_email'] ?? 'غير محدد'); ?>)
                                        <?php if (!empty($messageData['sender_phone'])): ?>
                                    ...
                                    <?php if ($messageData['status'] === 'replied' && !empty($messageData['reply_message'])): ?>

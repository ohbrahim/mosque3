    ...
    if ($action === 'add') {
        ...
        if (empty($title)) {
            ...
            try {
                ...
                $hasCreatedBy = false;
                $columnsQuery = $db->query("SHOW COLUMNS FROM pages");
                ...
                if ($hasCreatedBy) {
                    $stmt = $db->prepare("INSERT INTO pages (title, slug, content, status, created_by, created_at, updated_at) VALUES (?, ?, ?, ?, ?, NOW(), NOW())");
                    $stmt->execute([$title, $slug, $content, $status, $_SESSION['user_id']]);
                } else {
                    $stmt = $db->prepare("INSERT INTO pages (title, slug, content, status, created_at, updated_at) VALUES (?, ?, ?, ?, NOW(), NOW())");
...
try {
    ...
    // إنشاء الاستعلام المناسب
    if ($hasCreatedBy) {
        $stmt = $db->query("SELECT p.*, u.username as author FROM pages p LEFT JOIN users u ON p.created_by = u.id ORDER BY p.created_at DESC");
    } else {
        $stmt = $db->query("SELECT p.* FROM pages p ORDER BY p.created_at DESC");
    }
